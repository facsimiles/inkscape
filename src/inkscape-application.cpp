...
SPDesktop *InkscapeApplication::window_open(SPDocument *document)
{
...
void InkscapeApplication::window_close(SPDesktop *desktop)
{
    if (!desktop) {
    ...
    if (win->get_desktop_widget()->get_desktops().empty()) { // If last tab of window, close window.
        auto win_it = std::find_if(_windows.begin(), _windows.end(), [=] (auto &w) { return w.get() == win; });
 ...
 */
SPDesktop *InkscapeApplication::create_window(SPDocument *document, bool replace)
{
    if (!gtk_app()) {
...
void InkscapeApplication::create_window(Glib::RefPtr<Gio::File> const &file)
{
    ...
    if (file) {
        ...
        if (document) {
            ...
            desktop = create_window(document, replace);
 ...
 */
bool InkscapeApplication::destroy_window(SPDesktop *desktop, bool keep_alive)
{
    if (!gtk_app()) {
        g_assert_not_reached();
        return false;
...
void InkscapeApplication::detachTabToNewWindow(SPDesktop *desktop)
{
...
bool InkscapeApplication::destroy_all()
{
    ...
    while (!_documents.empty()) {
        ...
        if (!desktops.empty()) {
            if (!destroy_window(desktops.back().get())) {
...
void InkscapeApplication::process_document(SPDocument *document, std::string output_path)
{
    ...
    _active_document  = document;
    if (_with_gui) {
        _active_desktop = create_window(document, replace);
