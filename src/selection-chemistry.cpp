...
void SelectionHelper::fixSelection(SPDesktop *dt)
{
    ...
    for(auto i = boost::rbegin(selList); i != boost::rend(selList); ++i) {
        SPItem *item = *i;
        if( item &&
            !item->document->layerManager().isLayer(item) &&
            (!item->isLocked()))
        {
...
void ObjectSet::deleteItems(bool skip_undo)
{
    ...
    if (SPDesktop *dt = desktop()) {
        dt->getDocument()->layerManager().currentLayer()->requestDisplayUpdate(SP_OBJECT_MODIFIED_FLAG);
...
void ObjectSet::duplicate(bool suppressDone, bool duplicateLayer)
{
    ...
    if(duplicateLayer){
        reprs.clear();
        reprs.push_back(doc->layerManager().currentLayer()->getRepr());
    }
    ...
    if (!duplicateLayer) {
        ...
        if (auto label = new_layer->label()) {
            if (std::string(label).find("copy") == std::string::npos) {
                ...
                doc->layerManager().renameLayer( new_layer, name, TRUE );
                g_free(name);
            }
        }
    }
...
void sp_edit_clear_all(Inkscape::Selection *selection)
{
    ...
    auto group = doc->layerManager().currentLayer();
...
static void get_all_items_recursive(std::vector<SPItem*> &list, SPObject *from, SPDesktop *desktop, bool onlyvisible, bool onlysensitive, bool ingroups, std::vector<SPItem*> const &exclude)
{
    for (auto &child : from->children) {
        ...
        if (ingroups || (item && item->document->layerManager().isLayer(item))) {
...
static void sp_edit_select_all_full(SPDesktop *dt, bool force_all_layers, bool invert)
{
    ...
    switch (inlayer) {
        ...
        for (std::vector<SPItem*>::const_reverse_iterator i=all_items.rbegin();i!=all_items.rend();++i) {
            ...
            if (item && (!onlysensitive || !item->isLocked())) {
                if (!onlyvisible || !dt->itemIsHidden(item)) {
                    if (!item->document->layerManager().isLayer(item)) {
        ...
        case PREFS_SELECTION_LAYER_RECURSIVE: {
            items = get_all_items(dt->getDocument()->layerManager().currentLayer(), dt, onlyvisible, onlysensitive, FALSE, exclude);
            break;
        }
        default: {
            items = get_all_items(dt->getDocument()->layerManager().currentRoot(), dt, onlyvisible, onlysensitive, FALSE, exclude);
            break;
    }
    }
...
void ObjectSet::toNextLayer(bool skip_undo)
{
    ...
    if (next) {
        ...
        std::vector<Inkscape::XML::Node*> temp_clip;
        sp_selection_copy_impl(items_copy, temp_clip, dt->getDocument()->getReprDoc());
        sp_selection_delete_impl(items_copy, false, false);
        next=Inkscape::next_layer(dt->getDocument()->layerManager().currentRoot(), dt->getDocument()->layerManager().currentLayer()); // Fixes bug 1482973: crash while moving layers
        std::vector<Inkscape::XML::Node*> copied;
        if (next) {
        ...
        setReprList(copied);
        if (next) dt->getDocument()->layerManager().setCurrentLayer(next);
        if ( !skip_undo ) {
...
void ObjectSet::toPrevLayer(bool skip_undo)
{
    ...
    SPObject *next=Inkscape::previous_layer(dt->getDocument()->layerManager().currentRoot(), dt->getDocument()->layerManager().currentLayer());
    if (next) {
        clear();
        sp_selection_change_layer_maintain_clones(items_copy,next);
        std::vector<Inkscape::XML::Node*> temp_clip;
        sp_selection_copy_impl(items_copy, temp_clip, dt->getDocument()->getReprDoc()); // we're in the same doc, so no need to copy defs
        sp_selection_delete_impl(items_copy, false, false);
        next=Inkscape::previous_layer(dt->getDocument()->layerManager().currentRoot(), dt->getDocument()->layerManager().currentLayer()); // Fixes bug 1482973: crash while moving layers
        std::vector<Inkscape::XML::Node*> copied;
        if (next) {
            copied = sp_selection_paste_impl(dt->getDocument(), next, temp_clip);
        } else {
...
void ObjectSet::toLayer(SPObject *moveto, Inkscape::XML::Node *after)
{
    ...
    if (moveto) {
        ...
        setReprList(copied);
        if (!temp_clip.empty()) temp_clip.clear();
        if (moveto && dt) document()->layerManager().setCurrentLayer(moveto);
    }
}
...
void sp_select_same_fill_stroke_style(SPDesktop *desktop, gboolean fill, gboolean stroke, gboolean style)
{
    ...
    SPObject *root = desktop->getDocument()->layerManager().currentRoot();
    bool ingroup = true;
...
void sp_select_same_object_type(SPDesktop *desktop)
{
    ...
    bool onlysensitive = prefs->getBool("/options/kbselection/onlysensitive", true);
    bool ingroups = TRUE;
    auto matches = get_all_items(desktop->getDocument()->layerManager().currentRoot(), desktop, onlyvisible, onlysensitive, ingroups);
                  ...
                  bool only_in_viewport, PrefsSelectionContext inlayer, bool onlyvisible, bool onlysensitive)
{
    ...
    if (!path.empty()) {
        ...
        if (desktop->getDocument()->layerManager().isLayer(object)) {
    ...
    while ( !D::isNull(iter) && !found ) {
        ...
        if (desktop->getDocument()->layerManager().isLayer(object)) {
            ...
            if ( item &&
                 ...
                 ( !onlyvisible || !desktop->itemIsHidden(item)) &&
                 ( !onlysensitive || !item->isLocked()) &&
                 !desktop->getDocument()->layerManager().isLayer(item) )
            {
                found = item;
            }
        }
...
sp_selection_item_next(SPDesktop *desktop)
{
    ...
    if (PREFS_SELECTION_ALL != inlayer) {
        ...
        root = desktop->getDocument()->layerManager().currentRoot();
    }
 ...
 void ObjectSet::setMask(bool apply_clip_path, bool apply_to_layer, bool remove_original)
{
    ...
    if (apply_to_layer) {
        ...
        apply_to_items.push_back(desktop()->getDocument()->layerManager().currentLayer());
    }
...
static void itemtree_map(void (*f)(SPItem *, SPDesktop *), SPObject *root, SPDesktop *desktop) {
    // don't operate on layers
    {
        ...
        if (item && !desktop->getDocument()->layerManager().isLayer(item)) {
            f(item, desktop);
        }
    }
    for (auto& child: root->children) {
        ...
        auto item = cast<SPItem>(&child);
        if (!(item && desktop->getDocument()->layerManager().isLayer(item) && item->isLocked())) {
...
static void process_all(void (*f)(SPItem *, SPDesktop *), SPDesktop *dt, bool layer_only) {
    ...
    SPObject *root;
    if (layer_only) {
        root = dt->getDocument()->layerManager().currentLayer();
    } else {
        root = dt->getDocument()->layerManager().currentRoot();
    }
