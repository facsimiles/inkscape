...
ItemParam::ItemParam(Glib::ustring const &label, Glib::ustring const &tip,
                     Glib::ustring const &key, Inkscape::UI::Widget::Registry *wr,
                     Effect *effect, char const *default_value)
    : Parameter(label, tip, key, wr, effect)
    , ref(effect->getLPEObj())
{
...
void ItemParam::param_set_default()
{
    param_readSVGValue(defvalue);
}

void ItemParam::param_update_default(char const *default_value)
{
    defvalue = strdup(default_value);
}

void ItemParam::param_set_and_write_default()
{
...
SPObject *ItemParam::param_fork()
{
    ...
    auto oldobj = ref.getObject();
    if (!oldobj) {
        return nullptr;
    }

    auto xml_doc = document->getReprDoc();
    auto fork = oldobj->getRepr()->duplicate(xml_doc);
    auto newobj = oldobj->parent->appendChildRepr(fork);
    if (newobj && newobj->getId()) {
        linkitem(newobj->getId());
    }

    return newobj;
}

bool ItemParam::param_readSVGValue(char const *strvalue)
{
    if (!strvalue) {
        return false;
    }
...
Glib::ustring ItemParam::param_getSVGValue() const
{
    if (href) {
        return href;
    }
...
Glib::ustring ItemParam::param_getDefaultSVGValue() const
{
    return defvalue;
}
...
void ItemParam::addCanvasIndicators(SPLPEItem const *, std::vector<Geom::PathVector> &)
{
}


void ItemParam::start_listening(SPObject *to)
{
    if (!to) {
        return;
    }
...
void ItemParam::ref_changed(SPObject *, SPObject *new_ref)
{
    quit_listening();
    if (new_ref) {
...
void ItemParam::linked_delete(SPObject *)
{
    quit_listening();
    remove_link();
}

void ItemParam::linked_modified(SPObject *linked_obj, unsigned flags)
{
...
void ItemParam::linked_modified_callback(SPObject *linked_obj, unsigned)
{
    emit_changed();
...
void ItemParam::linked_transformed_callback(Geom::Affine const *rel_transf, SPItem *moved_item)
{
...
void ItemParam::linkitem(Glib::ustring itemid)
{
    if (itemid.empty()) {
        return;
    }
    ...
    if (href && std::strcmp(itemid.c_str(), href) == 0) {
...
void ItemParam::on_link_button_click()
{
