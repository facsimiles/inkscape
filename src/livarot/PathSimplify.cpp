 ...
 *              o-------------------------o
 *              start                    res.p
 * if the line is defined by l(t) 0 <= t <= 1, this distance between pt and any point on line
 * l(t) is defined as |l(t) - pt|. This function returns the minimum of this function over t.
 *
 * @param start The start point of the Cubic Bezier or line whatever.
 * @param res The Cubic Bezier desription which we really don't care about. We only use res.p.
...
void Path::DoSimplify(int off, int N, double treshhold)
{
  // non-dichotomic method: grow an interval of points approximated by a curve, until you reach the treshhold, and repeat
    ...
    while (curP < N - 1) {
        ...
        // We start with curP being 0 (the first point) and lastP being 1 (the second poin)
        ...
        // a cubic bezier description that we prepare and then add
...
bool Path::ExtendFit(int off, int N, fitting_tables &data, double treshhold, PathDescrCubicTo &res, int &worstP)
{
    ...
    if ( N > data.inPt ) {
        ...
        data.inPt = N; // update inPt to say okay we have added all the new points now
    }
     ...
     * consequences are not that terrible so we didn't notice this with results?
...
bool Path::AttemptSimplify(int off, int N, double treshhold, PathDescrCubicTo &res,int &worstP)
{
    ...
    lk[0] = 0.0;
    {
        // TODO: One problem here, why are we not storing Xk[0]? Is this a bug?
        ...
        for (int i = 1; i < N; i++) { // start the loop on the second point
            ...
            lk[i] = Geom::L2(diff);
            tk[i] = tk[i - 1] + lk[i]; // tk should have the length till this point, later we divide whole thing by total length to make time value
        }
    }
    ...
    if ( FitCubic(start, res, Xk, Yk, Qk, tk, N) ) { // fit a cubic, if goes well, set cp1 and cp2
        cp1 = start + res.start / 3; // this three factor comes from the fact that this is how livarot stores them. See docs in path-description.h
        cp2 = end + res.end / 3;
    } else {
        // aie, non-inversible
    ...
    double delta = 0;
    {
        ...
        if ( N <= 20 ) {
            for (int i = 1; i < N - 1; i++) // for every point that's not an endpoint
            {
                ...
                // it's a way of measuring error, so probably fine ¯\_(ツ)_/¯
