...
/**
 * An enum to store the path description type. A number is assigned to each type and the
...
enum
{
  ...
  descr_close = 5,          /*!< A close instruction */
...
/**
 * A base class for Livarot's Path Descriptions. Each curve type such as Line, CubicBezier
...
struct PathDescr
{
     ...
     * @param last The last point before this description. This is needed for the computation
    ...
    /**
     * A virtual function that derived classes will implement. Supposed to transform the path
     * description.
     *
     * It's simple to implement these for a MoveTo, LineTo and maybe even CubicTo. But I doubt
     * if it's done correctly with the Arc.
     *
     * @param t The Affine transformation matrix.
     */
...
/**
 * Intermediate Bezier Curve command.
 *
 ...
 * p0, p1, p2, p3 where p1 and p2 are control points. This is an instrucion so it's
...
struct PathDescrCubicTo : public PathDescr
{
  ...
  Geom::Point end;   /*!< 3 * (p3 - p2) where p3 is the final point of the cubic bezier and
                       p2 is thee second control point. */
};
...
/**
 * A forced point path description.
 *
...
struct PathDescrClose : public PathDescr
{
  ...
  ** Path::ConvertForcedToMoveTo suggests that maybe it should.
  */
  Geom::Point p; /*!< Probably useless since close instruction needs no point. */
};
