...
Glib::ustring box3d_side_axes_string(Box3DSide *side)
{
    ...
    axes_string += Box3D::string_from_axes ((Box3D::Axis) (side->dir1 ^ side->dir2));

    switch ((Box3D::Axis) (side->dir1 ^ side->dir2)) {
        case Box3D::XY:
            axes_string += ((side->front_or_rear == Box3D::FRONT) ? "front" : "rear");
            break;

        case Box3D::XZ:
            axes_string += ((side->front_or_rear == Box3D::FRONT) ? "top" : "bottom");
            break;

        case Box3D::YZ:
            axes_string += ((side->front_or_rear == Box3D::FRONT) ? "right" : "left");
            break;
