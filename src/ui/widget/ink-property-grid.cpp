...
void InkPropertyGrid::construct() {
    ...
    connectBeforeResize([this](int width, int height, int baseline) {
        ...
        if (single != _single_column) {
            ...
            if (abs(width - _single_column) < 2) return;
        }
...
void InkPropertyGrid::add_property(Gtk::Label* label, Gtk::Widget* button1, Gtk::Widget* w1, Gtk::Widget* w2, Gtk::Widget* btn, int margin) {
    if (label) {
    ...
    _min_width = m.sizes.minimum + 2;
}

void InkPropertyGrid::add_gap(int size) {
