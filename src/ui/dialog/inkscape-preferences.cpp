    ...
    , _init(true)
{
    ...
    page_list_selection->signal_changed().connect(
        sigc::mem_fun(*this, &InkscapePreferences::on_pagelist_selection_changed));
    ...
    _theme_oberver = prefs->createObserver("/theme/", [=]() { prefs->setString("/options/boot/theme", "custom"); });
}
...
Gtk::TreeModel::iterator InkscapePreferences::AddPage(DialogPage &p, Glib::ustring title,
                                                      Gtk::TreeModel::iterator parent, int id)
{
...
void InkscapePreferences::AddConvertGuidesCheckbox(DialogPage &p, Glib::ustring const &prefs_path, bool def_value)
{
    ...
    p.add_line(false, "", *cb, "",
               _("Converting an object to guides places these along the object's true edges (imitating the object's "
...
void InkscapePreferences::AddDotSizeSpinbutton(DialogPage &p, Glib::ustring const &prefs_path, double def_value)
{
    ...
    p.add_line(false, _("Ctrl+click _dot size:"), *sb, _("times current stroke width"),
               _("Size of dots created with Ctrl+click (relative to current stroke width)"), false);
}
...
void InkscapePreferences::AddBaseSimplifySpinbutton(DialogPage &p, Glib::ustring const &prefs_path, double def_value)
{
    ...
    p.add_line(false, _("Base simplify:"), *sb, _("on dynamic LPE simplify"),
               _("Base simplify of dynamic LPE based simplify"), false);
}
...
void InkscapePreferences::AddNewObjectsStyle(DialogPage &p, Glib::ustring const &prefs_path, const gchar *banner)
{
    ...
    p.add_line(true, "", *button, "", _("Remember the style of the (first) selected object as this tool's style"));
}
...
void InkscapePreferences::initPageTools()
{
    ...
    // shapes
    Gtk::TreeModel::iterator iter_shapes =
        this->AddPage(_page_shapes, _("Shapes"), iter_tools, PREFS_PAGE_TOOLS_SHAPES);
    ...
    _t_bbox_geometric.init(_("Geometric bounding box"), "/tools/bounding_box", 1, true,
                           &_t_bbox_visual); // 1 means geometric
    _page_tools.add_line(true, "", _t_bbox_geometric, "", _("This bounding box includes only the bare path"));
    ...
    _page_tools.add_line(
        true, "", _t_cvg_convert_whole_groups, "",
    ...
    _page_selector.add_line(true, "", _t_sel_trans_obj, "", _("Show the actual objects when moving or transforming"));
    ...
    _page_selector.add_line(true, "", _t_sel_cue_none, "", _("No per-object selection indication"));
    ...
    _page_selector.add_line(true, "", _t_sel_cue_box, "", _("Each selected object displays its bounding box"));
    ...
    _page_node.add_line(false, "", _t_node_pathoutline_color, "",
                        _("Selects the color used for showing the path outline"), false);
    ...
    _page_node.add_line(true, "", _t_node_live_outline, "",
                        _("Update the outline when dragging or transforming nodes; if this is off, the outline will "
                          "only update when completing a drag"));
    _t_node_live_objects.init(_("Update paths when dragging nodes"), "/tools/nodes/live_objects", false);
    _page_node.add_line(true, "", _t_node_live_objects, "",
                        _("Update paths when dragging or transforming nodes; if this is off, paths will only be "
    ...
    _page_node.add_line(
        true, "", _t_node_show_path_direction, "",
        _("Visualize the direction of selected paths by drawing small arrows in the middle of each outline segment"));
    _t_node_pathflash_enabled.init(_("Show temporary path outline"), "/tools/nodes/pathflash_enabled", false);
    _page_node.add_line(true, "", _t_node_pathflash_enabled, "",
                        _("When hovering over a path, briefly flash its outline"));
    _t_node_pathflash_selected.init(_("Show temporary outline for selected paths"), "/tools/nodes/pathflash_selected",
                                    false);
    _page_node.add_line(true, "", _t_node_pathflash_selected, "",
                        _("Show temporary outline even when a path is selected for editing"));
    _t_node_pathflash_timeout.init("/tools/nodes/pathflash_timeout", 0, 10000.0, 100.0, 100.0, 1000.0, true, false);
    _page_node.add_line(false, _("_Flash time:"), _t_node_pathflash_timeout, "ms",
                        _("Specifies how long the path outline will be visible after a mouse-over (in milliseconds); "
                          "specify 0 to have the outline shown until mouse leaves the path"),
                        false);
    _page_node.add_group_header(_("Editing preferences"));
    _t_node_single_node_transform_handles.init(_("Show transform handles for single nodes"),
                                               "/tools/nodes/single_node_transform_handles", false);
    _page_node.add_line(true, "", _t_node_single_node_transform_handles, "",
                        _("Show transform handles even when only a single node is selected"));
    _t_node_delete_preserves_shape.init(_("Deleting nodes preserves shape"), "/tools/nodes/delete_preserves_shape",
                                        true);
    _page_node.add_line(
        true, "", _t_node_delete_preserves_shape, "",
        _("Move handles next to deleted nodes to resemble original shape; hold Ctrl to get the other behavior"));
    ...
    _page_measure.add_line(
        false, "", *cb, "",
        _("The start and end of the measurement tool's control line will not be considered for calculating lengths. "
          "Only lengths between actual curve intersections will be displayed."));
    ...
    _page_pencil.add_line(true, "", _pencil_average_all_sketches, "",
                          _("If on, the sketch result will be the normal average of all sketches made, instead of "
    ...
    this->AddGradientCheckbox(_page_text, "/tools/text", true);
    {
        ...
        _page_text.add_line(false, "", *cb, "",
                            _("Show font samples alongside font names in the drop-down list in Text bar"));

        _font_dialog.init(_("Show font substitution warning dialog"), "/options/font/substitutedlg", false);
        _page_text.add_line(
            false, "", _font_dialog, "",
            _("Show font substitution warning dialog when requested fonts are not available on the system"));
        ...
        _page_text.add_line(false, "", *cb, "",
                            _("Use SVG2 auto-flowed text instead of SVG1.2 auto-flowed text. (Recommended)"));
    }
    ...
    //_page_text.add_line( true, "", _font_output_px, "", _("Always convert the text size units above into pixels (px)
    ...
    _page_text.add_line(
        true, "", _font_fontsdir_system, "",
        _("Load additional fonts from \"fonts\" directory located in Inkscape's global \"share\" directory"));
    _font_fontsdir_user.init(_("Use user's fonts directory"), "/options/font/use_fontsdir_user", true);
    _page_text.add_line(
        true, "", _font_fontsdir_user, "",
        _("Load additional fonts from \"fonts\" directory located in Inkscape's user configuration directory"));
    _font_fontdirs_custom.init("/options/font/custom_fontdirs", 50);
    _page_text.add_line(true, _("Additional font directories"), _font_fontdirs_custom, "",
                        _("Load additional fonts from custom locations (one path per line)"), true);
    ...
    _page_gradient.add_line(
        false, "", _misc_forkvectors, "",
        _("When on, shared gradient definitions are automatically forked on change; uncheck to allow sharing of "
          "gradient definitions so that editing one object may affect other objects using the same gradient"),
        true);
...
void InkscapePreferences::initPageUI()
{
    ...
    Glib::ustring languages[] = {_("System default"),
                                 ...
                                 _("Bulgarian (bg)"),
                                 _("Bengali (bn)"),
                                 _("Bengali/Bangladesh (bn_BD)"),
                                 _("Bodo (brx)"),
                                 _("Breton (br)"),
                                 _("Catalan (ca)"),
                                 _("Valencian Catalan (ca@valencia)"),
                                 _("Chinese/China (zh_CN)"),
                                 _("Chinese/Taiwan (zh_TW)"),
                                 _("Croatian (hr)"),
                                 _("Czech (cs)"),
                                 _("Danish (da)"),
                                 _("Dogri (doi)"),
                                 ...
                                 _("English/Canada (en_CA)"),
                                 _("English/Great Britain (en_GB)"),
                                 _("Esperanto (eo)"),
                                 _("Estonian (et)"),
                                 _("Farsi (fa)"),
                                 _("Finnish (fi)"),
                                 ...
                                 _("Japanese (ja)"),
                                 _("Kannada (kn)"),
                                 _("Kashmiri in Perso-Arabic script (ks@aran)"),
                                 _("Kashmiri in Devanagari script (ks@deva)"),
                                 _("Khmer (km)"),
                                 _("Kinyarwanda (rw)"),
                                 _("Konkani (kok)"),
                                 _("Konkani in Latin script (kok@latin)"),
                                 _("Korean (ko)"),
                                 _("Latvian (lv)"),
                                 _("Lithuanian (lt)"),
                                 _("Macedonian (mk)"),
                                 ...
                                 _("Malayalam (ml)"),
                                 _("Manipuri (mni)"),
                                 _("Manipuri in Bengali script (mni@beng)"),
                                 _("Marathi (mr)"),
                                 _("Mongolian (mn)"),
                                 _("Nepali (ne)"),
                                 _("Norwegian Bokmål (nb)"),
                                 ...
                                 _("Sanskrit (sa)"),
                                 _("Santali (sat)"),
                                 _("Santali in Devanagari script (sat@deva)"),
                                 _("Serbian (sr)"),
                                 _("Serbian in Latin script (sr@latin)"),
                                 _("Sindhi (sd)"),
                                 _("Sindhi in Devanagari script (sd@deva)"),
                                 _("Slovak (sk)"),
                                 _("Slovenian (sl)"),
                                 _("Spanish (es)"),
                                 _("Spanish/Mexico (es_MX)"),
                                 _("Swedish (sv)"),
                                 _("Tamil (ta)"),
                                 _("Telugu (te)"),
                                 _("Thai (th)"),
    ...
    Glib::ustring langValues[] = {
        "",          "sq",    "ar",  "hy",       "as",          "az",       "eu",      "be",      "bg",       "bn",
        "bn_BD",     "brx",   "br",  "ca",       "ca@valencia", "zh_CN",    "zh_TW",   "hr",      "cs",       "da",
        "doi",       "nl",    "dz",  "de",       "el",          "en",       "en_AU",   "en_CA",   "en_GB",    "eo",
        "et",        "fa",    "fi",  "fr",       "gl",          "gu",       "he",      "hi",      "hu",       "is",
        "id",        "ga",    "it",  "ja",       "kn",          "ks@aran",  "ks@deva", "km",      "rw",       "kok",
        "kok@latin", "ko",    "lv",  "lt",       "mk",          "mai",      "ml",      "mni",     "mni@beng", "mr",
        "mn",        "ne",    "nb",  "nn",       "or",          "pa",       "pl",      "pt",      "pt_BR",    "ro",
        "ru",        "sa",    "sat", "sat@deva", "sr",          "sr@latin", "sd",      "sd@deva", "sk",       "sl",
        "es",        "es_MX", "sv",  "ta",       "te",          "th",       "tr",      "uk",      "ur",       "vi"};

    {
        ...
        for (j = 1; j < n; j++) {
            ...
            i = j - 1;
            while (i >= 0 && ((languages[i] > key_language && langValues[i] != "") || key_langValue == "")) {
    ...
    _page_ui.add_line(false, _("Language:"), _ui_languages, "", _("Set the language for menus and number formats"),
                      false, reset_icon());
    ...
    _page_ui.add_group_note(
        _("Adjust the slider until the length of the ruler on your screen matches its real length. This information is "
    ...
    _page_ui.add_line(false, "", _ui_realworldzoom, "",
                      _("Zoom percentage can be either by the physical units or by pixels."));
    ...
    _page_ui.add_line(false, "", _ui_yaxisdown, "", _("When off, origin is at lower left corner and y-axis points up"),
                      false, reset_icon());

    _ui_rotationlock.init(_("Lock canvas rotation by default"), "/options/rotationlock", false);
    _page_ui.add_line(
        false, "", _ui_rotationlock, "",
    ...
    _page_ui.add_line(false, "", _narrow_spinbutton, "", _("Make number editing boxes smaller by limiting padding"),
                      false);
    _compact_colorselector.init(_("Use compact color selector mode switch"), "/colorselector/switcher", true);
    _page_ui.add_line(false, "", _compact_colorselector, "", _("Use compact combo box for selecting color modes"),
                      false);
    ...
    _page_ui.add_line(false, "", *sb_style, "",
                      _("Control visibility of current fill, stroke and opacity in status bar."), true);
    ...
    _page_ui.add_line(false, "", *sb_coords, "", _("Control visibility of mouse coordinates X & Y in status bar."),
                      true);
    ...
    _page_ui.add_line(
        false, "", _ui_cursorscaling, "",
        _("When off, cursor scaling is disabled. Cursor scaling may be broken when fractional scaling is enabled."),
        true);
    _ui_cursor_shadow.init(_("Show drop shadow"), "/options/cursor-drop-shadow", true);
    _page_ui.add_line(false, "", _ui_cursor_shadow, "", _("Control visibility of drop shadow for Inkscape cursors."),
                      true);
    ...
    _page_theme.add_line(true, _("User themes:"), _sys_user_themes_dir_copy, "", _("Location of the user’s themes"),
                         true, Gtk::manage(new Gtk::Box()));
    ...
    _page_theme.add_line(true, "", _dark_theme, "", _("Use dark theme"), true);
    {
        ...
        font_scale->getSlider()->signal_format_value().connect(
    ...
    _contrast_theme.getSlider()->signal_value_changed().connect(
        sigc::mem_fun(*this, &InkscapePreferences::contrastThemeChange));
    ...
    _page_theme.add_group_header(_("Icons"));
    {
        ...
        _sys_user_icons_dir_copy.init((char const *)IO::Resource::get_path(IO::Resource::USER, IO::Resource::ICONS, ""),
                                      _("Open icons folder"));
        _page_theme.add_line(true, _("User icons: "), _sys_user_icons_dir_copy, "", _("Location of the user’s icons"),
                             true, Gtk::manage(new Gtk::Box()));
    }
    Glib::ustring themeiconname = prefs->getString("/theme/iconTheme", prefs->getString("/theme/defaultIconTheme", ""));
    ...
    _symbolic_highlight_colors.init(_("Use default highlight colors for icons"), "/theme/symbolicDefaultHighColors",
                                    true);
    _symbolic_highlight_colors.signal_clicked().connect(
        sigc::mem_fun(*this, &InkscapePreferences::resetIconsColorsWrapper));
    ...
    _page_theme.add_line(false, "", *icon_buttons, _("Icon color base"), _("Base color for icons"), false);
    ...
    _page_theme.add_line(false, "", *icon_buttons_hight, _("Icon color highlights"),
                         _("Highlight colors supported by some symbolic icon themes"), false);
    Gtk::Box *icon_buttons_def = Gtk::manage(new Gtk::Box());
    resetIconsColors();
    changeIconsColor(0xffffffff);
    _page_theme.add_line(false, "", *icon_buttons_def, "", _("Reset theme colors for some symbolic icon themes"),
                         false);
    ...
    _page_theme.add_line(false, _("Show icons in menus:"), _menu_icons, "",
                         _("You can either enable or disable all icons in menus. By default, the setting for the "
                           "'use-icon' attribute in the 'menus.ui' file determines whether to display icons in menus."),
                         false, reset_icon());
    ...
    try {
        ...
        for (auto &&tbox : toolbars) {
            ...
            for (int i = min; i <= max; i += 8) {
                slider->getSlider()->add_mark(i, Gtk::POS_BOTTOM,
                                              i % min ? "" : (std::to_string(100 * i / min) + "%").c_str());
            }
        ...
        _page_toolbars.add_line(false, _("Snap controls bar:"),
                                *Gtk::make_managed<PrefRadioButtons>(snap, "/toolbox/simplesnap"), "", "");
    ...
    // Windows
    _win_save_geom.init(_("Save and restore window geometry for each document"), "/options/savewindowgeometry/value",
                        PREFS_WINDOW_GEOMETRY_FILE, true, nullptr);
    _win_save_geom_prefs.init(_("Remember and use last window's geometry"), "/options/savewindowgeometry/value",
                              PREFS_WINDOW_GEOMETRY_LAST, false, &_win_save_geom);
    _win_save_geom_off.init(_("Don't save window geometry"), "/options/savewindowgeometry/value",
                            PREFS_WINDOW_GEOMETRY_NONE, false, &_win_save_geom);
    ...
    _win_ontop_none.init(C_("Dialog on top", "None"), "/options/transientpolicy/value", PREFS_DIALOGS_WINDOWS_NONE,
                         false, nullptr);
    _win_ontop_normal.init(_("Normal"), "/options/transientpolicy/value", PREFS_DIALOGS_WINDOWS_NORMAL, true,
                           &_win_ontop_none);
    _win_ontop_agressive.init(_("Aggressive"), "/options/transientpolicy/value", PREFS_DIALOGS_WINDOWS_AGGRESSIVE,
                              false, &_win_ontop_none);

    _win_dialogs_labels_auto.init(_("Automatic"), "/options/notebooklabels/value", PREFS_NOTEBOOK_LABELS_AUTO, true,
                                  nullptr);
    _win_dialogs_labels_active.init(_("Active"), "/options/notebooklabels/value", PREFS_NOTEBOOK_LABELS_ACTIVE, true,
                                    nullptr);
    _win_dialogs_labels_off.init(_("Off"), "/options/notebooklabels/value", PREFS_NOTEBOOK_LABELS_OFF, false,
                                 &_win_dialogs_labels_auto);

    {
        Glib::ustring defaultSizeLabels[] = {C_("Window size", "Default"), C_("Window size", "Small"),
                                             C_("Window size", "Large"), C_("Window size", "Maximized")};
        int defaultSizeValues[] = {PREFS_WINDOW_SIZE_NATURAL, PREFS_WINDOW_SIZE_SMALL, PREFS_WINDOW_SIZE_LARGE,
                                   PREFS_WINDOW_SIZE_MAXIMIZED};

        _win_default_size.init("/options/defaultwindowsize/value", defaultSizeLabels, defaultSizeValues,
                               G_N_ELEMENTS(defaultSizeLabels), PREFS_WINDOW_SIZE_NATURAL);
    ...
    _page_windows.add_line(true, "", _win_native, "", _("Use Windows like open and save dialogs"));
    _page_windows.add_line(true, "", _win_gtk, "", _("Use GTK open and save dialogs "));
#endif
    _page_windows.add_group_header(_("Dialogs settings"), 4);

    std::vector<PrefItem> dock = {{_("Docked"), PREFS_DIALOGS_BEHAVIOR_DOCKABLE, _("Allow dialog docking"), true},
                                  {_("Floating"), PREFS_DIALOGS_BEHAVIOR_FLOATING, _("Disable dialog docking")}};
    _page_windows.add_line(true, _("Dialog behavior"),
                           *Gtk::make_managed<PrefRadioButtons>(dock, "/options/dialogtype/value"), "", "", false,
                           reset_icon());
    ...
    std::vector<PrefItem> on_top = {
        ...
        {_("Aggressive"), PREFS_DIALOGS_WINDOWS_AGGRESSIVE,
         _("Same as Normal but may work better with some window managers")}};
    _page_windows.add_line(true, _("Dialog on top"),
                           *Gtk::make_managed<PrefRadioButtons>(on_top, "/options/transientpolicy/value"), "", "");
#endif
    std::vector<PrefItem> labels = {{_("Automatic"), PREFS_NOTEBOOK_LABELS_AUTO,
                                     _("Dialog names will be displayed when there is enough space"), true},
    ...
    _page_windows.add_line(true, _("Labels behavior"),
                           *Gtk::make_managed<PrefRadioButtons>(labels, "/options/notebooklabels/value"), "", "", false,
                           reset_icon());
    ...
    _page_windows.add_line(true, "", _win_zoom_resize, "",
                           _("Zoom drawing when document window is resized, to keep the same area visible (this is the "
    ...
    _page_windows.add_line(true, "", _win_save_viewport, "",
                           _("Save documents viewport (zoom and panning position). Useful to turn off when sharing "
    ...
    _grids_no_emphasize_on_zoom.init(_("Minor grid line color"), "/options/grids/no_emphasize_when_zoomedout", 1, true,
                                     nullptr);
    _page_grids.add_line(true, "", _grids_no_emphasize_on_zoom, "",
                         _("The gridlines will be shown in minor grid line color"), false);
    _grids_emphasize_on_zoom.init(_("Major grid line color"), "/options/grids/no_emphasize_when_zoomedout", 0, false,
                                  &_grids_no_emphasize_on_zoom);
    _page_grids.add_line(true, "", _grids_emphasize_on_zoom, "",
                         _("The gridlines will be shown in major grid line color"), false);
    ...
    _grids_xy.add_line(false, _("Spacing X:"), _grids_xy_spacing_x, "", _("Distance between vertical grid lines"),
                       false);
    _grids_xy.add_line(false, _("Spacing Y:"), _grids_xy_spacing_y, "", _("Distance between horizontal grid lines"),
                       false);

    _grids_xy_color.init(_("Minor grid line color:"), "/options/grids/xy/color", GRID_DEFAULT_COLOR);
    _grids_xy.add_line(false, _("Minor grid line color:"), _grids_xy_color, "", _("Color used for normal grid lines"),
                       false);
    _grids_xy_empcolor.init(_("Major grid line color:"), "/options/grids/xy/empcolor", GRID_DEFAULT_EMPCOLOR);
    _grids_xy.add_line(false, _("Major grid line color:"), _grids_xy_empcolor, "",
                       _("Color used for major (highlighted) grid lines"), false);
    ...
    _grids_xy.add_line(false, "", _grids_xy_dotted, "", _("If set, display dots at gridpoints instead of gridlines"),
                       false);
    ...
    _grids_axonom.add_line(false, _("Minor grid line color:"), _grids_axonom_color, "",
                           _("Color used for normal grid lines"), false);
    _grids_axonom_empcolor.init(_("Major grid line color:"), "/options/grids/axonom/empcolor", GRID_DEFAULT_EMPCOLOR);
    _grids_axonom.add_line(false, _("Major grid line color:"), _grids_axonom_empcolor, "",
                           _("Color used for major (highlighted) grid lines"), false);
    ...
    _cp_show_full_action_name.init(_("Show command line argument names"),
                                   "/options/commandpalette/showfullactionname/value", false);
    _page_command_palette.add_line(true, "", _cp_show_full_action_name, "",
                                   _("Show action argument names in the command palette suggestions, most useful for "
                                     "using them on the command line"));

    _cp_show_untranslated_name.init(_("Show untranslated (English) names"),
                                    "/options/commandpalette/showuntranslatedname/value", true);
    _page_command_palette.add_line(true, "", _cp_show_untranslated_name, "",
                                   _("Also show the English names of the command"));
...
void InkscapePreferences::initPageIO()
{
    ...
    _save_use_current_dir.init(_("Use current directory for \"Save As ...\""), "/dialogs/save_as/use_current_dir",
                               true);
    _page_io.add_line(false, "", _save_use_current_dir, "",
                      _("When this option is on, the \"Save as...\" and \"Save a Copy...\" dialogs will always open in "
                        "the directory where the currently open document is; when it's off, each will open in the "
                        "directory where you last saved a file using it"),
                      true);

    _misc_default_metadata.init(_("Add default metadata to new documents"), "/metadata/addToNewFile", false);
    _page_io.add_line(
        false, "", _misc_default_metadata, "",
        _("Add default metadata to new documents. Default metadata can be set from Document Properties->Metadata."),
        true);
    ...
    _page_mouse.add_line(
        false, _("_Grab sensitivity:"), _mouse_sens, _("pixels"),
        _("How close on the screen you need to be to an object to be able to grab it with mouse (in screen pixels)"),
        false, reset_icon());
    ...
    _page_mouse.add_line(false, "", _mouse_use_ext_input, "",
                         _("Use the capabilities of a tablet or other pressure-sensitive device. Disable this only if "
                           "you have problems with the tablet (you can still use it as a mouse)"),
                         false, reset_icon());
    ...
    _page_mouse.add_line(false, "", _mouse_switch_on_ext_input, "",
                         _("Change tool as different devices are used on the tablet (pen, eraser, mouse)"), false,
                         reset_icon());
    ...
    _page_svgoutput.add_line(false, "", _svgoutput_usenamedcolors, "",
                             _("If set, write the CSS name of the color when available (e.g. 'red' or 'magenta') "
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_inlineattrs, "",
                             _("Put attributes on the same line as the element tag"), false);

    _svgoutput_indent.init("/options/svgoutput/indent", 0.0, 1000.0, 1.0, 2.0, 2.0, true, false);
    _page_svgoutput.add_line(
        true, _("_Indent, spaces:"), _svgoutput_indent, "",
        _("The number of spaces to use for indenting nested elements; set to 0 for no indentation"), false);
    ...
    _svgoutput_pathformat.init("/options/svgoutput/pathstring_format", pathstringFormatLabels, pathstringFormatValues,
                               numPathstringFormat, 2);
    _page_svgoutput.add_line(true, _("Path string format:"), _svgoutput_pathformat, "",
                             _("Path data should be written: only with absolute coordinates, only with relative "
    ...
    _page_svgoutput.add_line(
        true, "", _svgoutput_forcerepeatcommands, "",
        _("Force repeating of the same path command (for example, 'L 1,2 L 3,4' instead of 'L 1,2 3,4')"), false);
    ...
    _page_svgoutput.add_line(true, _("_Numeric precision:"), _svgoutput_numericprecision, "",
                             _("Significant figures of the values written to the SVG file"), false);

    _svgoutput_minimumexponent.init("/options/svgoutput/minimumexponent", -32.0, -1, 1.0, 2.0, -8.0, true, false);
    _page_svgoutput.add_line(true, _("Minimum _exponent:"), _svgoutput_minimumexponent, "",
                             _("The smallest number written to SVG is 10 to the power of this exponent; anything "
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_attrwarn, "",
                             _("Print warning if invalid or non-useful attributes found. Database files located in "
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_attrremove, "",
                             _("Delete invalid or non-useful attributes from element tag"), false);
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_stylepropwarn, "",
                             _("Print warning if inappropriate style properties found (i.e. 'font-family' set on a "
                               "<rect>). Database files located in inkscape_data_dir/attributes."),
                             false);
    _svgoutput_stylepropremove.init(_("Remove style properties"),
                                    "/options/svgoutput/incorrect_style_properties_remove", false);
    _page_svgoutput.add_line(true, "", _svgoutput_stylepropremove, "", _("Delete inappropriate style properties"),
                             false);
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_styledefaultswarn, "",
                             _("Print warning if redundant style properties found (i.e. if a property has the default "
                             ...
                             false);
    _svgoutput_styledefaultsremove.init(_("Remove style properties"), "/options/svgoutput/style_defaults_remove",
                                        false);
    _page_svgoutput.add_line(true, "", _svgoutput_styledefaultsremove, "", _("Delete redundant style properties"),
                             false);
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_check_reading, "",
                             _("Check attributes and style properties on reading in SVG files (including those "
    ...
    _page_svgoutput.add_line(true, "", _svgoutput_check_editing, "",
                             _("Check attributes and style properties while editing SVG files (may slow down Inkscape, "
                               "mostly useful for debugging)"),
                             false);
    _svgoutput_check_writing.init(_("Writing"), "/options/svgoutput/check_on_writing", true);
    _page_svgoutput.add_line(true, "", _svgoutput_check_writing, "",
                             _("Check attributes and style properties on writing out SVG files"), false);
    ...
    _svgexport_insert_text_fallback.init(_("Insert SVG 1.1 fallback in text"), "/options/svgexport/text_insertfallback",
                                         true);
    _svgexport_insert_mesh_polyfill.init(_("Insert JavaScript code for mesh gradients"),
                                         "/options/svgexport/mesh_insertpolyfill", true);
    _svgexport_insert_hatch_polyfill.init(_("Insert JavaScript code for SVG2 hatches"),
                                          "/options/svgexport/hatch_insertpolyfill", true);

    _page_svgexport.add_line(false, "", _svgexport_insert_text_fallback, "",
                             _("Adds fallback options for non-SVG 2 renderers."), false);
    _page_svgexport.add_line(false, "", _svgexport_insert_mesh_polyfill, "",
                             _("Adds a JavaScript polyfill for rendering meshes in web browsers."), false);
    _page_svgexport.add_line(false, "", _svgexport_insert_hatch_polyfill, "",
                             _("Adds a JavaScript polyfill for rendering hatches in web browsers."), false);

    // SVG Export Options (SVG 2 -> SVG 1)
    _page_svgexport.add_group_header(_("SVG 2 to SVG 1.1"));

    _svgexport_remove_marker_auto_start_reverse.init(_("Use correct marker direction in SVG 1.1 renderers"),
                                                     "/options/svgexport/marker_autostartreverse", false);
    _svgexport_remove_marker_context_paint.init(_("Use correct marker colors in SVG 1.1 renderers"),
                                                "/options/svgexport/marker_contextpaint", false);

    _page_svgexport.add_line(false, "", _svgexport_remove_marker_auto_start_reverse, "",
                             _("SVG 2 allows markers to automatically be reversed at the start of a path with "
                               "'auto_start_reverse'. This adds a rotated duplicate of the marker's definition."),
                             false);
    _page_svgexport.add_line(false, "", _svgexport_remove_marker_context_paint, "",
                             _("SVG 2 allows markers to automatically match the stroke color by using 'context_paint' "
                               "or 'context_fill'. This adjusts the markers own colors."),
                             false);
    ...
    Glib::ustring intentLabels[numIntents] = {_("Perceptual"), _("Relative Colorimetric"), _("Saturation"),
    ...
    gchar *profileTip = g_strdup_printf(
        _("The ICC profile to use to calibrate display output.\nSearched directories:%s"), tmpStr.c_str());
    _page_cms.add_line(true, _("Display profile:"), _cms_display_profile, "", profileTip, false);
    g_free(profileTip);
    profileTip = nullptr;
    ...
    _page_cms.add_line(true, "", _cms_softproof, "", _("Simulates output of target device"), false);

    _cms_gamutwarn.init(_("Mark out of gamut colors"), "/options/softproof/gamutwarn", false);
    _page_cms.add_line(true, "", _cms_gamutwarn, "", _("Highlights colors that are out of gamut for the target device"),
                       false);
    ...
    _page_cms.add_line(true, "", _cms_proof_blackpoint, "", _("Enables black point compensation"), false);
    ...
    _cms_display_profile.signal_changed().connect(
        sigc::bind(sigc::ptr_fun(profileComboChanged), &_cms_display_profile));
    ...
    _page_autosave.add_line(
        false, "", _save_autosave_enable, "",
        _("Automatically save the current document(s) at a given interval, thus minimizing loss in case of a crash"),
        false);
    ...
    _page_autosave.add_line(false, C_("Filesystem", "Autosave _directory:"), _save_autosave_path, "",
    ...
    _page_autosave.add_line(false, _("_Interval (in minutes):"), _save_autosave_interval, "",
                            _("Interval (in minutes) at which document will be autosaved"), false);
    _save_autosave_max.init("/options/autosave/max", 1.0, 10000.0, 1.0, 10.0, 10.0, true, false);
    _page_autosave.add_line(false, _("_Maximum number of autosaves:"), _save_autosave_max, "",
                            _("Maximum number of autosaved files; use this to limit the storage space used"), false);
...
void InkscapePreferences::initPageBehavior()
{
    ...
    _page_behavior.add_line(false, _("_Simplification threshold:"), _misc_simpl, "",
                            _("How strong is the Node tool's Simplify command by default. If you invoke this command "
    ...
    _markers_color_stock.init(_("Color stock markers the same color as object"), "/options/markers/colorStockMarkers",
                              true);
    _markers_color_custom.init(_("Color custom markers the same color as object"),
                               "/options/markers/colorCustomMarkers", false);
    _markers_color_update.init(_("Update marker color when object color changes"),
                               "/options/markers/colorUpdateMarkers", true);

    // Selecting options
    _sel_all.init(_("Select in all layers"), "/options/kbselection/inlayer", PREFS_SELECTION_ALL, false, nullptr);
    _sel_current.init(_("Select only within current layer"), "/options/kbselection/inlayer", PREFS_SELECTION_LAYER,
                      true, &_sel_all);
    _sel_recursive.init(_("Select in current layer and sublayers"), "/options/kbselection/inlayer",
                        PREFS_SELECTION_LAYER_RECURSIVE, false, &_sel_all);
    ...
    _sel_touch_topmost_only.init(_("Select the topmost items only when in touch selection mode"),
                                 "/options/selection/touchsel_topmost_only", true);

    _page_select.add_line(
        false, "", _sel_layer_deselects, "",
        _("Uncheck this to be able to keep the current objects selected when the current layer changes"));

    _page_select.add_line(
        false, "", _sel_touch_topmost_only, "",
    ...
    _page_select.add_line(true, "", _sel_all, "", _("Make keyboard selection commands work on objects in all layers"));
    ...
    _page_select.add_line(true, "", _sel_hidden, "",
                          _("Uncheck this to be able to select objects that are hidden (either by themselves or by "
                            "being in a hidden layer)"));
    _page_select.add_line(true, "", _sel_locked, "",
                          _("Uncheck this to be able to select objects that are locked (either by themselves or by "
                            "being in a locked layer)"));
    _page_select.add_line(true, "", _sel_inlayer_same, "",
                          _("Check this to make the 'select same' functions work like the select all functions, "
                            "restricting to current layer only."));
    ...
    _page_select.add_line(true, "", _sel_cycle, "", _("Wrap around at start and end when cycling objects in z-order"));
    ...
    _page_scrolling.add_line(
        false, _("Mouse _wheel scrolls by:"), _scroll_wheel, _("pixels"),
    ...
    _page_scrolling.add_line(
        true, _("_Acceleration:"), _scroll_arrow_acc, "",
    ...
    _page_scrolling.add_line(
        true, _("_Speed:"), _scroll_auto_speed, "",
    ...
    _page_scrolling.add_line(true, _("_Threshold:"), _scroll_auto_thres, _("pixels"),
                             _("How far (in screen pixels) you need to be from the canvas edge to trigger autoscroll; "
    ...
    _page_snapping.add_line(true, _("Snap indicator persistence (in seconds):"), _snap_persistence, "",
                            _("Controls how long the snap indicator message will be shown, before it disappears"),
                            true);

    _snap_indicator_distance.init(_("Show snap distance in case of alignment or distribution snap"),
                                  "/options/snapindicatordistance/value", false);
    ...
    _page_snapping.add_line(
        true, _("_Weight factor:"), _snap_weight, "",
        _("When multiple snap solutions are found, then Inkscape can either prefer the closest transformation (when "
          "set to 0), or prefer the node that was initially the closest to the pointer (when set to 1)"),
        true);

    _snap_mouse_pointer.init(_("Snap the mouse pointer when dragging a constrained knot"),
                             "/options/snapmousepointer/value", false);
    _page_snapping.add_line(true, "", _snap_mouse_pointer, "",
                            _("When dragging a knot along a constraint line, then snap the position of the mouse "
    ...
    _page_snapping.add_line(true, _("Delay (in seconds):"), _snap_delay, "",
                            _("Postpone snapping as long as the mouse is moving, and then wait an additional fraction "
    ...
    _page_steps.add_line(false, "", _steps_compass, "",
                         _("When on, angles are displayed with 0 at north, 0 to 360 range, positive clockwise; "
                           "otherwise with 0 at east, -180 to 180 range, positive counterclockwise"));
    int const num_items = 18;
    Glib::ustring labels[num_items] = {"90", "60", "45", "36", "30",  "22.5",
                                       "18", "15", "12", "10", "7.5", "6",
                                       "5",  "3",  "2",  "1",  "0.5", C_("Rotation angle", "None")};
    ...
    _page_steps.add_line(
        false, _("_Rotation snaps every:"), _steps_rot_snap, _("degrees"),
        _("Rotating with Ctrl pressed snaps every that much degrees; also, pressing [ or ] rotates by this amount"),
        false);
    _steps_rot_relative.init(_("Relative snapping of guideline angles"), "/options/relativeguiderotationsnap/value",
                             false);
    _page_steps.add_line(
        false, "", _steps_rot_relative, "",
    ...
    _clone_option_stay.init(_("Stay unmoved"), "/options/clonecompensation/value", SP_CLONE_COMPENSATION_UNMOVED, false,
    ...
    _clone_option_unlink.init(_("Are unlinked"), "/options/cloneorphans/value", SP_CLONE_ORPHANS_UNLINK, true, nullptr);
    _clone_option_delete.init(_("Are deleted"), "/options/cloneorphans/value", SP_CLONE_ORPHANS_DELETE, false,
    ...
    _page_clones.add_line(true, "", _clone_option_transform, "",
                          _("Each clone moves according to the value of its transform= attribute; for example, a "
    ...
    _page_clones.add_line(true, "", _clone_option_unlink, "", _("Orphaned clones are converted to regular objects"));
    ...
    _page_clones.add_line(
        true, "", _clone_relink_on_duplicate, "",
        _("When duplicating a selection containing both a clone and its original (possibly in groups), relink the "
    ...
    _page_clones.add_line(true, "", _clone_to_curves, "",
                          _("The following path operations will unlink clones: Stroke to path, Object to path, Boolean "
    ...
    _mask_mask_on_top.init(_("When applying, use the topmost selected object as clippath/mask"),
                           "/options/maskobject/topmost", true);
    _page_mask.add_line(false, "", _mask_mask_on_top, "",
                        _("Uncheck this to use the bottom selected object as the clipping path or mask"));
    _mask_mask_on_ungroup.init(_("When ungroup, clip/mask is preserved in children"),
                               "/options/maskobject/maskonungroup", true);
    _page_mask.add_line(false, "", _mask_mask_on_ungroup, "", _("Uncheck this to remove clip/mask on ungroup"));
    ...
    _mask_grouping_none.init(_("Do not group clipped/masked objects"), "/options/maskobject/grouping",
                             PREFS_MASKOBJECT_GROUPING_NONE, true, nullptr);
    _mask_grouping_separate.init(_("Put every clipped/masked object in its own group"), "/options/maskobject/grouping",
                                 PREFS_MASKOBJECT_GROUPING_SEPARATE, false, &_mask_grouping_none);
    _mask_grouping_all.init(_("Put all clipped/masked objects into one group"), "/options/maskobject/grouping",
                            PREFS_MASKOBJECT_GROUPING_ALL, false, &_mask_grouping_none);

    _page_mask.add_line(true, "", _mask_grouping_none, "", _("Apply clippath/mask to every object"));

    _page_mask.add_line(true, "", _mask_grouping_separate, "",
                        _("Apply clippath/mask to groups containing single object"));

    _page_mask.add_line(true, "", _mask_grouping_all, "", _("Apply clippath/mask to group containing all objects"));
    ...
    _page_mask.add_line(true, "", _mask_ungrouping, "", _("Ungroup groups created when setting clip/mask"));
    ...
    _page_markers.add_line(true, "", _markers_color_custom, "",
                           _("Stroke color same as object, fill color either object fill color or marker fill color"));
    _page_markers.add_line(true, "", _markers_color_update, "", _("Update marker color when object color changes"));
    ...
    _cleanup_swatches.init(_("Remove unused swatches when doing a document cleanup"), "/options/cleanupswatches/value",
                           false); // text label
    ...
    _page_lpe.add_line(true, "", _lpe_copy_mirroricons, "",
                       _("Enables using 16 advanced mirror options between the copies (so there can be copies that are "
                         "mirrored differently between the rows and the columns) for Tiling LPE")); // tooltip
...
{
    /* threaded blur */ // related comments/widgets/functions should be renamed and option should be moved elsewhere
                        // when inkscape is fully multi-threaded
    _filter_multi_threaded.init("/options/threading/numthreads", 1.0, 8.0, 1.0, 2.0, 4.0, true, false);
    _page_rendering.add_line(false, _("Number of _Threads:"), _filter_multi_threaded, "",
    ...
    _page_rendering.add_line(false, _("Rendering _cache size:"), _rendering_cache_size,
                             C_("mebibyte (2^20 bytes) abbreviation", "MiB"),
                             _("Set the amount of memory per document which can be used to store rendered parts of the "
    ...
    _page_rendering.add_line(false, _("Rendering tile multiplier:"), _rendering_tile_multiplier, "",
                             _("On modern hardware, increasing this value (default is 16) can help to get a better "
    ...
    _rendering_outline_overlay_opacity.init("/options/rendering/outline-overlay-opacity", 1.0, 100.0, 1.0, 5.0, 50.0,
                                            true, false);
    _rendering_outline_overlay_opacity.signal_focus_out_event().connect(
        sigc::mem_fun(*this, &InkscapePreferences::on_outline_overlay_changed));
    ...
    // update strategy
    {
        ...
        _page_rendering.add_line(false, _("Update strategy:"), _canvas_update_strategy, "",
                                 _("How to update continually changing content when it can't be redrawn fast enough"),
                                 false);
    }

    // opengl
    _canvas_request_opengl.init("", "/options/rendering/request_opengl", false);
    _page_rendering.add_line(false, _("Enable OpenGL:"), _canvas_request_opengl, "",
                             _("Request that the canvas should be painted with OpenGL rather than Cairo. If OpenGL is "
                               "unsupported, it will fall back to Cairo."),
                             false);

    /* blur quality */
    _blur_quality_best.init(_("Best quality (slowest)"), "/options/blurquality/value", BLUR_QUALITY_BEST, false,
                            nullptr);
    _blur_quality_better.init(_("Better quality (slower)"), "/options/blurquality/value", BLUR_QUALITY_BETTER, false,
                              &_blur_quality_best);
    _blur_quality_normal.init(_("Average quality"), "/options/blurquality/value", BLUR_QUALITY_NORMAL, true,
                              &_blur_quality_best);
    _blur_quality_worse.init(_("Lower quality (faster)"), "/options/blurquality/value", BLUR_QUALITY_WORSE, false,
                             &_blur_quality_best);
    _blur_quality_worst.init(_("Lowest quality (fastest)"), "/options/blurquality/value", BLUR_QUALITY_WORST, false,
                             &_blur_quality_best);

    _page_rendering.add_group_header(_("Gaussian blur quality for display"));
    _page_rendering.add_line(
        true, "", _blur_quality_best, "",
        _("Best quality, but display may be very slow at high zooms (bitmap export always uses best quality)"));
    _page_rendering.add_line(true, "", _blur_quality_better, "", _("Better quality, but slower display"));
    _page_rendering.add_line(true, "", _blur_quality_normal, "", _("Average quality, acceptable display speed"));
    ...
    _page_rendering.add_line(
        true, "", _filter_quality_best, "",
        _("Best quality, but display may be very slow at high zooms (bitmap export always uses best quality)"));
    _page_rendering.add_line(true, "", _filter_quality_better, "", _("Better quality, but slower display"));
    _page_rendering.add_line(true, "", _filter_quality_normal, "", _("Average quality, acceptable display speed"));
    ...
    _page_rendering.add_line(false, "", _cairo_dithering, "",
                             _("Makes gradients smoother. This can significantly impact the size of generated PNG "
    ...
    _canvas_developer_mode_enabled.signal_toggled().connect(
        [revealer, this] { revealer->set_reveal_child(_canvas_developer_mode_enabled.get_active()); });
    _page_rendering.add_group_header(_("Developer mode"));
    _page_rendering.add_line(true, "", _canvas_developer_mode_enabled, "", _("Enable additional debugging options"),
                             false);
    _page_rendering.add(*revealer);

    auto add_devmode_line = [&](Glib::ustring const &label, Gtk::Widget &widget, Glib::ustring const &suffix,
    ...
    auto add_devmode_group_header = [&](Glib::ustring name) {
        auto label_widget = Gtk::make_managed<Gtk::Label>(Glib::ustring(/*"<span size='large'>*/ "<b>") + name +
                                                              Glib::ustring("</b>" /*</span>"*/),
    ...
    _canvas_render_time_limit.init("/options/rendering/render_time_limit", 100.0, 1000000.0, 1.0, 0.0, 1000.0, true,
                                   false);
    add_devmode_line(_("Render time limit"), _canvas_render_time_limit, C_("microsecond abbreviation", "μs"),
                     _("The maximum time allowed for a rendering time slice"));
    _canvas_use_new_bisector.init("", "/options/rendering/use_new_bisector", true);
    add_devmode_line(_("Use new bisector algorithm"), _canvas_use_new_bisector, "",
                     _("Use an alternative, more obvious bisection strategy: just chop tile in half along the larger "
                       "dimension until small enough"));
    _canvas_new_bisector_size.init("/options/rendering/new_bisector_size", 1.0, 10000.0, 1.0, 0.0, 500.0, true, false);
    add_devmode_line(_("Smallest tile size for new bisector"), _canvas_new_bisector_size,
                     C_("pixel abbreviation", "px"),
                     _("Halve rendering tile rectangles until their largest dimension is this small"));
    _rendering_tile_size.init("/options/rendering/tile-size", 1.0, 10000.0, 1.0, 0.0, 16.0, true, false);
    add_devmode_line(_("Tile size:"), _rendering_tile_size, "",
                     _("The \"tile size\" parameter previously hard-coded into Inkscape's original tile bisector."));
    _canvas_pad.init("/options/rendering/pad", 0.0, 1000.0, 1.0, 0.0, 350.0, true, false);
    add_devmode_line(_("Buffer padding"), _canvas_pad, C_("pixel abbreviation", "px"),
                     _("Use buffers bigger than the window by this amount"));
    ...
    add_devmode_line(_("Preempt size"), _canvas_preempt, "",
                     _("Prevent thin tiles at the rendering edge by making them at least this size."));
    _canvas_coarsener_min_size.init("/options/rendering/coarsener_min_size", 0.0, 1000.0, 1.0, 0.0, 200.0, true, false);
    add_devmode_line(_("Min size for coarsener algorithm"), _canvas_coarsener_min_size, C_("pixel abbreviation", "px"),
                     _("Coarsener algorithm only processes rectangles smaller/thinner than this."));
    _canvas_coarsener_glue_size.init("/options/rendering/coarsener_glue_size", 0.0, 1000.0, 1.0, 0.0, 80.0, true,
                                     false);
    add_devmode_line(_("Glue size for coarsener algorithm"), _canvas_coarsener_glue_size,
                     C_("pixel abbreviation", "px"),
                     _("Coarsener algorithm absorbs nearby rectangles within this distance."));
    _canvas_coarsener_min_fullness.init("/options/rendering/coarsener_min_fullness", 0.0, 1.0, 0.0, 0.0, 0.3, false,
                                        false);
    add_devmode_line(_("Min fullness for coarsener algorithm"), _canvas_coarsener_min_fullness, "",
                     _("Refuse coarsening algorithm's attempt if the result would be more empty than this."));
    {
        int values[] = {1, 2, 3, 4};
        ...
        add_devmode_line(_("Pixel streaming method"), _canvas_pixelstreamer_method, "",
                         _("Change the method used for streaming pixel data to the GPU. The default is Auto, which "
                           "picks the best method available at runtime. As for the other options, higher up is better. "
                           "Be warned! No attempt is made to stop you from selecting a method that isn't supported! "
                           "(This is dev mode, afer all.) If you do so, it will be an instant crash."));
    }
    ...
    add_devmode_line(_("Framecheck"), _canvas_debug_framecheck, "",
                     _("Print profiling data of selected operations to a file"));
    ...
    _canvas_debug_slow_redraw_time.init("/options/rendering/debug_slow_redraw_time", 0.0, 1000000.0, 1.0, 0.0, 50.0,
                                        true, false);
    add_devmode_line(_("Slow redraw time"), _canvas_debug_slow_redraw_time, C_("microsecond abbreviation", "μs"),
                     _("The delay to introduce for each tile"));
    _canvas_debug_show_redraw.init("", "/options/rendering/debug_show_redraw", false);
    add_devmode_line(_("Show redraw"), _canvas_debug_show_redraw, "",
                     _("Paint a translucent random colour over each newly drawn tile"));
    _canvas_debug_show_unclean.init("", "/options/rendering/debug_show_unclean", false);
    add_devmode_line(_("Show unclean region"), _canvas_debug_show_unclean, "",
                     _("Show the region that needs to be redrawn in red (only in Cairo mode)"));
    _canvas_debug_show_snapshot.init("", "/options/rendering/debug_show_snapshot", false);
    add_devmode_line(_("Show snapshot region"), _canvas_debug_show_snapshot, "",
                     _("Show the region that still contains a saved copy of previously rendered content in blue (only "
                       "in Cairo mode)"));
    _canvas_debug_show_clean.init("", "/options/rendering/debug_show_clean", false);
    add_devmode_line(_("Show clean region's fragmentation"), _canvas_debug_show_clean, "",
                     _("Show the outlines of the rectangles in the region where rendering is complete in green (only "
                       "in Cairo mode)"));
    _canvas_debug_disable_redraw.init("", "/options/rendering/debug_disable_redraw", false);
    add_devmode_line(_("Disable redraw"), _canvas_debug_disable_redraw, "",
                     _("Temporarily disable the idle redraw process completely"));
    _canvas_debug_sticky_decoupled.init("", "/options/rendering/debug_sticky_decoupled", false);
    add_devmode_line(_("Sticky decoupled mode"), _canvas_debug_sticky_decoupled, "",
                     _("Stay in decoupled mode even after rendering is complete"));
    _canvas_debug_animate.init("", "/options/rendering/debug_animate", false);
    add_devmode_line(_("Animate"), _canvas_debug_animate, "",
                     _("Continuously adjust viewing parameters in an animation loop."));
    _canvas_debug_idle_starvation.init("", "/options/rendering/debug_idle_starvation", false);
    add_devmode_line(_("Print render time stats"), _canvas_debug_idle_starvation, "",
                     _("On display of each frame, log to the console how much time was taken away from rendering, and "
                       "whether rendering is still busy. A high value would explain lag/fragmentation problems, and a "
                       "low value with 'still busy' would explain tearing."));
...
void InkscapePreferences::initPageBitmaps()
{
    ...
    _importexport_export_res.init("/dialogs/export/defaultxdpi/value", 0.0, 6000.0, 1.0, 1.0,
                                  Inkscape::Util::Quantity::convert(1, "in", "px"), true, false);
    ...
    _bitmap_copy_res.init("/options/createbitmap/resolution", 1.0, 6000.0, 1.0, 1.0,
                          Inkscape::Util::Quantity::convert(1, "in", "px"), true, false);
    ...
    _page_bitmaps.add_line(true, "", _svg_ask, "", _("Pop-up linking and scaling dialog when importing SVG image."));
    ...
    _page_bitmaps.add_line(
        ...
        _("By default, image links are stored as relative paths whenever possible. If this option is enabled, Inkscape "
          ...
          "fall-back for locating the linked image, for example if the SVG document has been moved on disk. Note that "
          "this "
          "will expose your directory structure in the file's source code, which can include personal information like "
          "your username."),
        false);
    ...
    _importexport_import_res.init("/dialogs/import/defaultxdpi/value", 0.0, 6000.0, 1.0, 1.0,
                                  Inkscape::Util::Quantity::convert(1, "in", "px"), true, false);
    ...
    _page_bitmaps.add_line(false, "", _rendering_image_outline, "",
                           _("When active will render images while in outline mode instead of a red box with an x. "
...
void InkscapePreferences::initKeyboardShortcuts(Gtk::TreeModel::iterator iter_ui)
{
    ...
    _kb_store->set_sort_column(GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID,
                               Gtk::SORT_ASCENDING); // only sort in onKBListKeyboardShortcuts()
    ...
    _kb_tree.get_column(1)->set_cell_data_func(_kb_shortcut_renderer,
                                               sigc::ptr_fun(InkscapePreferences::onKBShortcutRenderer));
    ...
    _mod_tree.get_selection()->signal_changed().connect(
        sigc::mem_fun(*this, &InkscapePreferences::on_modifier_selection_changed));
    ...
    kb_reset->set_tooltip_text(_("Remove all your customized keyboard shortcuts, and revert to the shortcuts in the "
...
void InkscapePreferences::onKBTreeEdited(const Glib::ustring &path, guint accel_key, Gdk::ModifierType accel_mods,
                                         guint hardware_keycode)
{
    ...
    if (!new_shortcut_key.is_null() && (new_shortcut_key.get_key() != current_shortcut_key.get_key() ||
                                        new_shortcut_key.get_mod() != current_shortcut_key.get_mod())) {
        // Check if there is currently an actions assigned to this shortcut; if yes ask if the shortcut should be
        // reassigned
...
bool InkscapePreferences::onKBSearchFilter(const Gtk::TreeModel::const_iterator &iter)
{
    ...
    return (name.lowercase().find(search) != name.npos || shortcut.lowercase().find(search) != name.npos ||
            desc.lowercase().find(search) != name.npos || id.lowercase().find(search) != name.npos);
}
...
void InkscapePreferences::on_modifier_edited()
{
    ...
    if (!_kb_mod_enabled.get_active()) {
        ...
        if (_kb_mod_ctrl.get_active())
            mask |= Inkscape::Modifiers::CTRL;
        if (_kb_mod_shift.get_active())
            mask |= Inkscape::Modifiers::SHIFT;
        if (_kb_mod_alt.get_active())
            mask |= Inkscape::Modifiers::ALT;
        if (_kb_mod_meta.get_active())
            mask |= Inkscape::Modifiers::META;
...
void InkscapePreferences::onKBListKeyboardShortcuts()
{
    ...
    auto action_sort = [&](Glib::ustring &a, Glib::ustring &b) {
    ...
    for (auto modifier : Inkscape::Modifiers::Modifier::getList()) {
        ...
        (*iter_modifier)[_mod_columns.name] =
            (modifier->get_name() && strlen(modifier->get_name())) ? _(modifier->get_name()) : "";
        (*iter_modifier)[_mod_columns.id] = modifier->get_id();
        (*iter_modifier)[_mod_columns.description] =
            (modifier->get_description() && strlen(modifier->get_description())) ? _(modifier->get_description()) : "";
...
void InkscapePreferences::initPageSpellcheck()
{
    ...
    _page_spellcheck.add_line(false, "", _spell_ignoreallcaps, "", _("Ignore words in all capitals, such as \"IUPAC\""),
                              true);
...
void InkscapePreferences::initPageSystem()
{
    ...
    _page_system.add_line(false, _("Latency _skew:"), _misc_latency_skew, "",
                          _("Factor by which the event clock is skewed from the actual time (0.9766 on some systems)"),
                          false, reset_icon());
    Inkscape::Preferences *prefs = Inkscape::Preferences::get();
    ...
    _page_system.add_line(false, "", _misc_namedicon_delay, "",
                          _("When on, named icons will be rendered before displaying the ui. This is for working "
    ...
    _sys_user_extension_dir.init(extensions_folder, _("Open extensions folder"));
    ...
    _page_system.add_line(true, _("Temporary files:"), _sys_tmp_files, "",
                          _("Location of the temporary files used for autosave"), true);
    ...
    _page_system.add_line(true, _("Inkscape extensions:"), _sys_extension_dir, "",
                          _("Location of the Inkscape extensions"), true);
    ...
    _page_system.add_line(true, _("Custom Font directories"), _sys_fontdirs_custom, "",
                          _("Load additional fonts from custom locations (one path per line)"), true);
...
void InkscapePreferences::on_reset_open_recent_clicked()
{
    ...
    for (auto e : recent_list) {
        if (e->has_application(g_get_prgname()) || e->has_application("org.inkscape.Inkscape") ||
