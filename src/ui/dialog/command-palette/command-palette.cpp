    ...
    , _CPHistoryScroll(get_widget<Gtk::ScrolledWindow>(_builder, "CPHistoryScroll"))
{
    ...
    // Setup operations [actions, extensions]
    {
        // setup recent files
        {
            ...
            for (auto const &recent_file : recent_files) {
                ...
                bool valid_file =
                    recent_file->has_application(prgname) or recent_file->has_application("org.inkscape.Inkscape") or
                    recent_file->has_application("inkscape") or recent_file->has_application("inkscape.exe");

                // Note: Do not check if the file exists, to avoid long delays. See
                // https://gitlab.com/inkscape/inkscape/-/issues/2348 .
    ...
    // History management
    {
        auto const history = _history_xml.get_operation_history();

        for (auto const &page : history) {
...
void CommandPalette::append_recent_file_operation(Glib::ustring const &path, bool is_suggestion, bool is_import)
{
    ...
    Glib::ustring const file_name = file->get_basename();
    ...
    CPName.set_tooltip_text((is_import ? ("Import") : ("Open")) + (": " + file_name)); // Tooltip_text are not
                                                                                       // translatable
...
bool CommandPalette::generate_action_operation(ActionPtrName const &action_ptr_name, bool is_suggestion)
{
    static auto const app = InkscapeApplication::instance();
    static InkActionExtraData const &action_data = app->get_action_extra_data();
    static bool const show_full_action_name =
    ...
    if (not show_full_action_name) {
        ...
        CPActionFullButton.signal_clicked().connect(
            sigc::bind(sigc::mem_fun(*this, &CommandPalette::on_action_fullname_clicked), action_ptr_name.second),
            false);
    }
...
void CommandPalette::on_action_fullname_clicked(Glib::ustring const &action_fullname)
{
...
void CommandPalette::on_row_activated(Gtk::ListBoxRow *activated_row)
{
    ...
    if (full_action_name == "import" or full_action_name == "open") {
        auto const [name, description] = get_name_desc(activated_row);
...
void CommandPalette::on_history_selection_changed(Gtk::ListBoxRow *lb)
{
    ...
    if (auto const name_label = get_name_desc(lb).first; name_label) {
...
bool CommandPalette::operate_recent_file(Glib::ustring const &uri, bool const import)
{
    ...
    if (!UI::get_children(_CPHistory).empty()) {
        if (auto const last_operation = _history_xml.get_last_operation(); last_operation.has_value()) {
            if (uri.raw() == last_operation->data) {
                bool last_operation_was_import = last_operation->history_type == HistoryKind::IMPORT_FILE;
 ...
 */
bool CommandPalette::ask_action_parameter(ActionPtrName const &action_ptr_name)
{
    _ask_action_ptr_name.emplace(action_ptr_name);

    // Avoid writing same last action again
    // TODO: Merge the if else parts
    if (auto const last_of_history = _history_xml.get_last_operation(); last_of_history.has_value()) {
        // operation history is not empty
        auto const last_full_action_name = last_of_history->data;
 ...
 * Color removal
 */
void CommandPalette::remove_color(Gtk::Label *label, Glib::ustring const &subject, bool tooltip)
{
    /* if (tooltip) {
 ...
 * Color addition
 */
Glib::ustring make_bold(Glib::ustring const &search)
{
    // TODO: Add a CSS class that changes the color of the search
    return "<span weight=\"bold\">" + search + "</span>";
}

void CommandPalette::add_color(Gtk::Label *label, Glib::ustring const &search, Glib::ustring const &subject,
                               bool tooltip)
{
 ...
 */
void CommandPalette::add_color_description(Gtk::Label *label, Glib::ustring const &search)
{
...
bool CommandPalette::fuzzy_tolerance_search(Glib::ustring const &subject, Glib::ustring const &search)
{
    Glib::ustring subject_string = subject.lowercase();
    Glib::ustring search_string = search.lowercase();
    std::map<gunichar, int> subject_string_character, search_string_character;
    for (auto const &character : subject_string) {
        subject_string_character[character]++;
    }
    for (auto const &character : search_string) {
        search_string_character[character]++;
    }
    for (auto const &character : search_string_character) {
        auto const &[alphabet, occurrence] = character;
...
bool CommandPalette::fuzzy_search(Glib::ustring const &subject, Glib::ustring const &search)
{
    Glib::ustring subject_string = subject.lowercase();
    Glib::ustring search_string = search.lowercase();
 ...
 */
bool CommandPalette::normal_search(Glib::ustring const &subject, Glib::ustring const &search)
{
 ...
 */
int CommandPalette::fuzzy_points(Glib::ustring const &subject, Glib::ustring const &search)
{
...
int CommandPalette::fuzzy_tolerance_points(Glib::ustring const &subject, Glib::ustring const &search)
{
    ...
    switch (mode) {
        ...
        case CPMode::HISTORY: {
            ...
            Glib::signal_idle().connect_once([this] {
                auto const adjustment = _CPHistoryScroll.get_vadjustment();
...
CommandPalette::ActionPtrName CommandPalette::get_action_ptr_name(Glib::ustring full_action_name)
{
    ...
    auto const win = InkscapeApplication::instance()->get_active_window();
    auto const doc = InkscapeApplication::instance()->get_active_document();
    auto const dot = full_action_name.find('.');
    auto const action_domain = std::string_view{full_action_name.c_str(), dot}; // app, win, doc
    auto const action_name = full_action_name.substr(dot + 1);
    ...
    if (action_domain == "app") {
        ...
        if (auto const map = doc->getActionGroup(); map) {
...
bool CommandPalette::execute_action(ActionPtrName const &action_ptr_name, Glib::ustring const &value)
{
    if (!value.empty()) {
        _history_xml.add_action_parameter(action_ptr_name.second, value);
    }

    auto const &[action_ptr, action_name] = action_ptr_name;
    ...
    switch (get_action_variant_type(action_ptr)) {
        case TypeOfVariant::BOOL:
            if (value == "1" || value == "t" || value == "true" || value.empty()) {
                ...
                std::cerr << "CommandPalette::execute_action: Invalid boolean value: " << action_name.raw() << ":"
                          << value << std::endl;
            }
            break;
        ...
        case TypeOfVariant::UNKNOWN:
            std::cerr << "CommandPalette::execute_action: unhandled action value type (Unknown Type) "
                      << action_name.raw() << std::endl;
            break;
...
TypeOfVariant CommandPalette::get_action_variant_type(ActionPtr const &action_ptr)
{
    ...
    if (gtype) {
        ...
        if (type.get_string() == "b") {
            ...
            std::cerr << "CommandPalette::get_action_variant_type: unknown variant type: " << type.get_string()
                      << std::endl;
