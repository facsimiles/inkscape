...
void KnotHolder::knot_enter_handler(SPKnot *knot, unsigned state)
{
    for(auto e : entity) {
        if (e->knot == knot) {
            e->knot_enter(state);
...
void KnotHolder::knot_leave_handler(SPKnot *knot, unsigned state)
{
    for(auto e : entity) {
        if (e->knot == knot) {
            e->knot_leave(state);
...
bool KnotHolder::knot_event_handler(SPKnot *knot, Inkscape::CanvasEvent const &event)
{
    for(auto e : entity) {
        if (e->knot == knot) {
