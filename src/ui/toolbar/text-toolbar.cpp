...
void TextToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        ...
        _fontsUpdatedConn = fontlister->connectNewFonts([=] {
            fontlister->update_font_list(desktop->getDocument());
        });

        auto sel = desktop->getSelection();
        _selectionChangedConn = sel->connectChangedFirst(sigc::mem_fun(*this, &TextToolbar::_selectionChanged));
        _selectionModifiedConn = sel->connectModifiedFirst(sigc::mem_fun(*this, &TextToolbar::_selectionModified));
        _cursorMovedConn = desktop->connect_text_cursor_moved([this] (Tools::TextTool *tool) {
            _cursorMoved(tool);
        });
        ...
        for (auto combo : {_font_style_item, _font_size_item, _font_family_item}) {
            combo->setDefocusWidget(_desktop->getCanvas());
        }
        for (auto btn : {&_line_height_item, &_letter_spacing_item, &_word_spacing_item, &_dx_item, &_dy_item, &_rotation_item}) {
