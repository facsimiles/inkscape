...
void StarToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        ...
        _selectionChangedConn = sel->connectChanged(sigc::mem_fun(*this, &StarToolbar::_selectionChanged));
        _selectionChanged(sel); // Synthesize an emission to trigger the update

        _magnitude_item.set_defocus_widget(_desktop->getCanvas());
        _spoke_item.set_defocus_widget(_desktop->getCanvas());
        _roundedness_item.set_defocus_widget(_desktop->getCanvas());
        _randomization_item.set_defocus_widget(_desktop->getCanvas());
    }
}
