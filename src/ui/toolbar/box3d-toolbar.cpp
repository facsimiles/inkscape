...
void Box3DToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        ...
        _selectionChangedConn = sel->connectChanged(sigc::mem_fun(*this, &Box3DToolbar::_selectionChanged));
        _selectionChanged(sel); // Synthesize an emission to trigger the update

        _angle_x_item.set_defocus_widget(_desktop->getCanvas());
        _angle_y_item.set_defocus_widget(_desktop->getCanvas());
        _angle_z_item.set_defocus_widget(_desktop->getCanvas());
    }
}
