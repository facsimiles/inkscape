...
void SpiralToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        ...
        _selectionChangedConn = sel->connectChanged(sigc::mem_fun(*this, &SpiralToolbar::_selectionChanged));
        _selectionChanged(sel); // Synthesize an emission to trigger the update

        _revolution_item.set_defocus_widget(_desktop->getCanvas());
        _expansion_item.set_defocus_widget(_desktop->getCanvas());
        _t0_item.set_defocus_widget(_desktop->getCanvas());
    }
}
