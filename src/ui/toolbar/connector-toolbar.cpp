...
void ConnectorToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        _selectionChangedConn = _desktop->getSelection()->connectChanged(sigc::mem_fun(*this, &ConnectorToolbar::_selectionChanged));
        ...
        _curvature_item.set_defocus_widget(_desktop->getCanvas());
        _spacing_item.set_defocus_widget(_desktop->getCanvas());
        _length_item.set_defocus_widget(_desktop->getCanvas());
    }
}
