...
void ArcToolbar::setDesktop(SPDesktop *desktop)
{
    ...
    if (_desktop) {
        ...
        _selectionChangedConn = sel->connectChanged(sigc::mem_fun(*this, &ArcToolbar::_selectionChanged));
        _selectionChanged(sel); // Synthesize an emission to trigger the update

        if (!_unit_set) {
            _tracker->setActiveUnit(_desktop->getNamedView()->getDisplayUnit());
            _unit_set = true;
        }

        _rx_item.set_defocus_widget(_desktop->getCanvas());
        _ry_item.set_defocus_widget(_desktop->getCanvas());
        _start_item.set_defocus_widget(_desktop->getCanvas());
        _end_item.set_defocus_widget(_desktop->getCanvas());
...
void ArcToolbar::_setDefaults()
{
    ...
    if (auto canvas = _desktop->getCanvas()) {
        canvas->grab_focus();
    }
}
