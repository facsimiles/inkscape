 ...
 */
 ...
 void TextTool::_setupText()
 {
     ...
     // Apply nascent style if it exists
     if (nascent_style) {
         sp_repr_css_merge_and_set(rtext, nascent_style, "style");
         sp_repr_css_attr_unref(nascent_style);
         nascent_style = nullptr;
     }
                     ...
                     switch (group0_keyval) {
                         ...
                         case GDK_KEY_b:
                             if (mod_ctrl_only(event) && text) {
                                 auto const style = sp_te_style_at_position(text, text_sel_start);
 ...
 */
 ...
 {
     if (!text) {
         // If no text exists, store the style for future text creation
         nascent_style = sp_repr_css_attr_clone(css);
         return true;
     }
 
     if (text_sel_start == text_sel_end) {
         // Apply style to the current cursor position
         sp_te_apply_style(text, text_sel_start, text_sel_start, css);
     } else {
         // Apply style to the selected text
         sp_te_apply_style(text, text_sel_start, text_sel_end, css);
     }
 
     // Rebuild layout and update representation
