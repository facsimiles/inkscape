...
public:
    ...
    static ProtocolSession simulate_session(const std::vector<std::string>& commands) {
    ...
    static bool validate_session(const ProtocolSession& session) {
        // Check handshake
        if (session.received_responses.empty() || 
            session.received_responses[0] != "WELCOME:Client ID 1") {
            return false;
        }
    ...
    static bool test_scenario(const TestScenario& scenario) {
    ...
    static bool is_valid_response_format(const std::string& response) {
    ...
    static bool matches_response_pattern(const std::string& response, const std::string& pattern) {
        if (pattern.empty()) {
    ...
    static std::string simulate_command_response(const std::string& command, int client_id) {
        ...
        if (command.find("COMMAND:") == 0) {
            ...
            if (parts.size() >= 3) {
                ...
                if (action == "status") {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":SUCCESS:0:Document active - Size: 800x600px, Objects: 0";
                } else if (action == "action-list") {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":OUTPUT:0:file-new,add-rect,export-png,status,action-list";
                } else if (action == "file-new") {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":SUCCESS:0:Command executed successfully";
                } else if (action == "add-rect") {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":SUCCESS:0:Command executed successfully";
                } else if (action == "export-png") {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":SUCCESS:0:Command executed successfully";
                } else {
                    return "RESPONSE:" + std::to_string(client_id) + ":" + request_id + ":ERROR:2:No valid actions found";
                }
            }
        }
    ...
    static std::vector<TestScenario> create_test_scenarios() {
        ...
        scenario3.commands = {
            ...
            "COMMAND:790:add-rect:100:100:200:200",
...
TEST_F(SocketIntegrationTestFixture, ResponseFormatValidation) {
    EXPECT_TRUE(SocketIntegrationTest::is_valid_response_format("RESPONSE:1:123:SUCCESS:0:Command executed successfully"));
...
TEST_F(SocketIntegrationTestFixture, ResponsePatternMatching) {
    EXPECT_TRUE(SocketIntegrationTest::matches_response_pattern("RESPONSE:1:123:SUCCESS:0:Command executed", "SUCCESS"));
    ...
    EXPECT_FALSE(SocketIntegrationTest::matches_response_pattern("RESPONSE:1:123:SUCCESS:0:Command executed", "FAILURE"));
    EXPECT_TRUE(SocketIntegrationTest::matches_response_pattern("RESPONSE:1:123:SUCCESS:0:Command executed", "")); // Empty pattern
}
...
TEST_F(SocketIntegrationTestFixture, ComplexIntegrationScenarios) {
    ...
    std::vector<std::string> workflow_commands = {
        "COMMAND:100:status",
        "COMMAND:101:action-list",
        "COMMAND:102:file-new",
        "COMMAND:103:add-rect:50:50:100:100",
    ...
    for (const auto& response : session.received_responses) {
